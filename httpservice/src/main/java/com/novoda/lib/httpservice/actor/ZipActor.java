...
public class ZipActor extends LoggingActor {
	...
	public void onPreprocess(HttpUriRequest method, HttpContext context) {
		Cursor c = null;
		try {
			c = getHttpContext().getContentResolver().query(IntentModel.URI, null, "_id = ?", new String[]{"" + getIntent().filterHashCode()}, null);
			if(c.moveToNext()) {
		...
		String range = "bytes=" + currentLenght + "-" + (currentLenght + fileReader.getThreshold() +1);
...
//		Log.v("Content Encoding : " + response.getEntity().getContentEncoding());
